...
MorsApp::MorsApp(int & argc, char** argv)
    : QApplication(argc, argv)
{
    ...
    QString optionPath = qMorsApp()->applicationDirPath() + "/smark.qconfig";
...
MorsApp::~MorsApp(void) {
    this->saveOption();
}

//! 重设为默认设置
void MorsApp::loadDefaultOption(void)
{
    ...
    QString localMathJaxUrl = qMorsApp()->applicationDirPath() + "/mathjax/MathJax.js";
    ...
    QString localCSSUrl = qMorsApp()->applicationDirPath() + "/smark.css";
...
//! 获取指定 key 的设置
QString MorsApp::option(const QString& key) const {
    return options_.value(key);
}

//! 设置指定 key 的设置值
void MorsApp::setOption(const QString& key, const QString& val) {
    options_[key] = val;
}

//! 保存当前设置
void MorsApp::saveOption(void) const {
    QString optionPath = qMorsApp()->applicationDirPath() + "/smark.qconfig";
    QFile   optionFile(optionPath);
